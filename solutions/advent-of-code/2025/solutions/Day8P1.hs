...
main = do
    ...
    let (finalUF, _) = processEdges uf edges 0 1000
...
processEdges :: UnionFind -> [(Double, Int, Int)] -> Int -> Int -> (UnionFind, Int)
processEdges uf [] _ _ = (uf, 0)
processEdges uf _ connections maxConnections
    | connections >= maxConnections = (uf, connections)
processEdges uf ((_, i, j):rest) connections maxConnections = 
    let newUF = union uf i j
    in processEdges newUF rest (connections + 1) maxConnections
